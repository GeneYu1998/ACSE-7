...
def figure6():
    """Figure showing orthogonal projection along search direction line."""
    fig, ax = plt.subplots(1, 1, figsize=(12, 8))
    x1 = np.array([0,0])
    pk = np.array([6,-2])
    x2 = x1 + 0.75*pk
    xs = x2 + [-0.5*pk[1], 0.5*pk[0]]

    ax.plot([x1[0], x1[0]+pk[0]], [x1[1], x1[1]+pk[1]], 'r', linewidth=2)
    ax.plot([x2[0], xs[0]], [x2[1], xs[1]], 'k--')
    ax.plot([x1[0], xs[0]], [x1[1], xs[1]], '--', color='grey')

    ax.plot(x1[0], x1[1], 'k.', markersize=10)
    ax.plot(x2[0], x2[1], 'k.', markersize=10)
    ax.plot(xs[0], xs[1], 'k.', markersize=10)

    ax.annotate(r"$\bf x^{(k)}$", xy=x1, fontsize=20,
                horizontalalignment='right', verticalalignment='top')
    ax.annotate(r"$\bf x^{(k+1)}$", xy=x2-0.01*pk, fontsize=20,
                horizontalalignment='right', verticalalignment='top')
    ax.annotate(r"$\bf x^{\ast}$", xy=xs, fontsize=20,
                horizontalalignment='left', verticalalignment='bottom')

    def arrow(ax, x, dx):
        ax.plot(x[0], x[1], 'k.')
        ax.annotate("", xy=x+dx, xytext=x,
               arrowprops={'arrowstyle': '->,head_width=0.5', 'linewidth': 2})

    arrow(ax, x1, pk*0.25)
    ax.annotate(r"$\bf p^{(k)}$", xy=x1+0.2*pk, fontsize=20,
                horizontalalignment='left', verticalalignment='bottom')
